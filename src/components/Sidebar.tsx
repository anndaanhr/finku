...
const navItems = [
  { href: '/dashboard', icon: 'grid_view', label: 'Dashboard', exact: true },
  { href: '/dashboard/transactions', icon: 'swap_horiz', label: 'Transaksi', exact: false },
  { href: '/dashboard/categories', icon: 'bookmarks', label: 'Kategori', exact: false },
  { href: '/dashboard/settings', icon: 'manage_accounts', label: 'Pengaturan', exact: false },
]
    ...
    <aside style={{
      ...
      <div style={{ padding: '28px 20px 20px', borderBottom: '2px solid rgba(255,255,255,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, background: '#FFE500', border: '2px solid #FFE500', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '3px 3px 0 rgba(255,255,255,0.15)' }}>
            <span className="material-symbols-outlined" style={{ fontSize: 20, color: '#0A0A0A', fontVariationSettings: "'FILL' 1" }}>north_star</span>
          </div>
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: '#FFE500', letterSpacing: '-0.5px' }}>FinKu</div>
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.2em', marginTop: -3 }}>KEUANGAN PRIBADI</div>
          </div>
        </div>
      </div>

      {/* User card */}
      <div style={{ margin: '16px 14px', padding: '14px', borderRadius: 12, background: 'rgba(255,229,0,0.1)', border: '2px solid rgba(255,229,0,0.25)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 9, background: '#FFE500', border: '2px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, color: '#0A0A0A', flexShrink: 0 }}>{initials}</div>
          <div style={{ overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2 }}>
              ...
              <span style={{ fontSize: 10, color: '#00C853', fontWeight: 700 }}>AKTIF</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '0 20px 10px' }}>
        <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.2em', color: 'rgba(255,255,255,0.25)' }}>MENU UTAMA</p>
      </div>
      ...
      <div style={{ padding: '16px 10px', borderTop: '2px solid rgba(255,255,255,0.1)' }}>
        ...
        <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.15)', textAlign: 'center', marginTop: 12, fontWeight: 600 }}>FinKu v1.0 · 2026</p>
      </div>
    </aside>
  )
}
